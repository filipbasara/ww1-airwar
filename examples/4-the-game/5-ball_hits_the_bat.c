...
float ball_hits_bat(int bat, int ballx)
{
    // which part of the bat did it it?
    // bats are divided in 5 zones horizontally, where the ball hits influences the X axis movement
    // of the ricocheting ball in different ways... 
    for(int z=0; z<5; z++)
    {
        if(ballx >= obj[bat].rect.x + z * (obj[bat].rect.width / 5) && 
           ballx <= obj[bat].rect.x + z * (obj[bat].rect.width / 5) + obj[bat].rect.width / 5 )
        {
            if(z==0)    return((float) (rand() % 18 + 1) / 10.0 * -1 );    
            if(z==1)    return((float) (rand() % 9 + 1)  / 10.0 * -1);    
            if(z==2)    return((float) (rand() % 4 + 1)  / 10.0 );    
            if(z==3)    return((float) (rand() % 9 + 1)  / 10.0 );    
            if(z==4)    return((float) (rand() % 18 + 1) / 10.0 );    
        } 
    }
    ...
    for(int other=0; other<117; other++)
    {
        if(which != other) // ball does not collide with itself...
        {
            ...
            {                                  
                ...
                {
                    // hits a brick: 
                    // - lets remove it from our screen
                    // - add points to player score
                    if(obj[other].role==1) ball_hits_brick(other);

                    // - invert movement on X axis
                    obj[which].speed.x *= -1;
                }
            ...
            {                                  
                if(CheckCollisionRecs(temprect, obj[other].rect) )
                {
                    ...
                    if(obj[other].role==0) 
                    {
                        obj[which].speed.x += ball_hits_bat(other, temprect.x);
                        if(obj[which].speed.x > 5) obj[which].speed.x=5.0;
                    }

                    // - invert movement on Y axis & except when hitting side walls
                    if(obj[other].role != 3) obj[which].speed.y *= -1;
                }
            }
        }
    }                    
...
int main(void)
{
    ...
    // seed the random numbers generator with time
